...
def read_data_2(train_data_dir, test_data_dir):
    clients = []
    train_data = {}
    test_data = {}

    for u in os.listdir(train_data_dir):
        train_data[u[:8]] = {}
        clients.append(u[:8])
        train_data[u[:8]] = np.load(os.path.join(train_data_dir, u))

    for u in os.listdir(test_data_dir):
        test_data[u[:8]] = {}
        test_data[u[:8]] = np.load(os.path.join(test_data_dir, u))
