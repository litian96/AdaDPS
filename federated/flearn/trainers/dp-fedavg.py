...
class Server(BaseFedarated):
    def __init__(self, params, learner, dataset):
        print('Using DP Federated avg to Train')
        self.inner_opt = tf.train.GradientDescentOptimizer(params['learning_rate'])
        super(Server, self).__init__(params, learner, dataset)
        self.G = []
        for params in self.latest_model:
            self.G.append(np.zeros_like(params))  
        self.delta = 1.0 / len(self.clients)

    def train(self):
        '''Train using DP Federated Average'''
        ...
        for i in range(self.num_rounds):
            ...
            if i % self.eval_every == 0:
                ...
                compute_dp_sgd_privacy(len(self.clients), self.clients_per_round, self.sigma, self.clients_per_round * i / len(self.clients) , self.delta)
            ...
            for idx, c in enumerate(selected_clients):  # simply drop the slow devices
                ...
                cupdates.append(clipped)

            sum_updates = self.simple_sum(cupdates)
            noise = []
            for p in sum_updates:
                noise.append(np.random.normal(0, self.sigma * self.clipping_bound, size=p.shape))

            # add noise, divided by the number of clients per round
            noisy_updates = [(u + v) / self.clients_per_round for u, v in zip(sum_updates, noise)]
            self.latest_model = [(u+v) for u, v in zip(weights_before, noisy_updates)]
